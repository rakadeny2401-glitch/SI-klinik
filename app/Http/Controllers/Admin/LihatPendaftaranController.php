...
class LihatPendaftaranController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = DB::table('daftar as d')
            ...
            ->leftJoin('proses_pasien as pp', 'd.id_daftar', '=', 'pp.id_daftar')
            ->select(
                'd.*', 
                's.nama_spesialis',
                'doc.nama_dokter',
        ...
        $data = $query->orderByDesc('d.id_daftar')->get();
    ...
    public function confirm(Request $request)
{
    $id = $request->id_value;
    if (!$id) {
        return back()->with('error', "ID Pendaftaran tidak diterima.");
    }
    ...
    // PENTING: Definisi variabel waktu daftar
    $waktuDaftar = strtotime($daftar->waktu_daftar);
    ...
    try {
        ...
        // 3. Waktu pemeriksaan = waktu daftar + 30 menit (SELALU)
        $tgl_final = $waktuDaftar + (30 * 60);
        ...
        DB::table('proses_pasien')->updateOrInsert(
            ...
            [
                ...
                'tgl_pemeriksaan' => date('Y-m-d H:i:s', $tgl_final),
        ...
        DB::rollBack();
        // Return error ke user agar tahu apa yang salah
