...
class PendaftaranController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // Waktu pemeriksaan = waktu daftar + 30 menit (SELALU)
            $tgl_pemeriksaan = date('Y-m-d H:i:s', strtotime($fullTime) + (30 * 60));
            
            // Hitung urutan berdasarkan total antrian di spesialis tersebut
            ...
            DB::rollBack(); \Log::error('Error Pendaftaran: ' . $e->getMessage() . ' | Line: ' . $e->getLine());
    ...
    {
        $request->validate(['id_value' => 'required']);

        DB::beginTransaction();
        try {
            $daftar = Daftar::findOrFail($request->id_value);
            $daftar->status_pendaftaran = 'dikonfirmasi';
            $daftar->save();

            // 1. Ambil waktu yang diinput di form (konversi ke timestamp)
            $waktuInputForm = strtotime($daftar->waktu_daftar);

            // Waktu pemeriksaan = waktu daftar + 30 menit (SELALU)
            $tgl_pemeriksaan_timestamp = $waktuInputForm + (30 * 60);

            // Hitung nomor antrian
            $sp = Spesialis::find($daftar->id_spesialis);
            $kode = strtoupper(substr($sp->nama_spesialis, 0, 1));
            $urutan = ProsesPasien::where('id_spesialis', $daftar->id_spesialis)->count() + 1;
            $noAntrian = $kode . '-' . str_pad($urutan, 3, '0', STR_PAD_LEFT);
            ...
            ProsesPasien::create([
                ...
                'tgl_pemeriksaan' => date('Y-m-d H:i:s', $tgl_pemeriksaan_timestamp),
                'no_antrian'      => $noAntrian
            ]);
