    ...
    public function confirm(Request $request)
{
    ...
    try {
        ...
        $lastProses = ProsesPasien::join(
                'daftar',
                'daftar.id_daftar',
                '=',
                'proses_pasien.id_daftar'
            )
            ->where('proses_pasien.id_spesialis', $daftar->id_spesialis)
            ->whereIn('daftar.status_pendaftaran', [
                'dikonfirmasi',
                'pemeriksaan'
            ])
            ->orderBy('proses_pasien.tgl_pemeriksaan', 'desc')
        ...
        if (!$lastProses) {
            ...
            $lastNomor = (int) preg_replace('/[^0-9]/', '', $lastProses->no_antrian);

            $urutan = $lastNomor + 1;

            if (strtotime($fullTime) > strtotime($lastProses->tgl_pemeriksaan)) {
                $tgl_pemeriksaan = $fullTime;
            } else {
                $tgl_pemeriksaan = date(
                    'Y-m-d H:i:s',
                    strtotime('+30 minutes', strtotime($lastProses->tgl_pemeriksaan))
                );
            }
        }
        ...
        return back()->with(
            'success',
            'Konfirmasi berhasil, antrean dan jadwal pemeriksaan telah dibuat.'
        );
    } catch (\Throwable $e) {
        DB::rollBack();
